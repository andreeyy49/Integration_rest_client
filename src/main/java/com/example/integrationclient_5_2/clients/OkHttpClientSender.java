package com.example.integrationclient_5_2.clients;
...
public class OkHttpClientSender {
    ...
    public String uploadFile(MultipartFile file) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                log.error("Error try to request to upload file!");
                return "Error!";
            }
    ...
    public Resource downloadFile(String filename) {
        ...
        try (Response response = httpClient.newCall(request).execute()) {
            if (!response.isSuccessful()) {
                log.error("Error try to download file!");
                return null;
            }
    ...
    public EntityModel createEntity(UpsertEntityRequest request){
        ...
        return processResponse(httpRequest, new TypeReference<EntityModel>() {});
    }
